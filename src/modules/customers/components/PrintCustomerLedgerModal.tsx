    ...
    <div className="fixed inset-0 bg-black/60 z-[1000] flex justify-center items-center backdrop-blur-sm print:bg-white print:p-0 overflow-y-auto">
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-5xl flex flex-col max-h-[95vh] print:max-h-none print:shadow-none print:rounded-none">
        ...
        <div className="flex-1 overflow-y-auto bg-gray-50 p-8 print:p-0 print:bg-white">
          <div id="customer-ledger-print-area" className={`bg-white shadow-sm mx-auto print:shadow-none min-h-[297mm] w-full text-black font-sans ${isA5 ? 'p-6 max-w-[148mm] min-h-[210mm]' : 'p-12 max-w-[210mm] min-h-[297mm]'}`}>
            <div className="flex flex-col items-center border-b-2 border-black pb-4 mb-6 text-center">
                ...
                <div className="w-full flex justify-between items-center mt-4 pt-2 border-t border-dashed border-gray-300">
                    <h2 className={`${isA5 ? 'text-[10px] px-2 py-0.5' : 'text-xs px-3 py-0.5'} font-black uppercase tracking-widest bg-black text-white`}>Account Statement</h2>
            ...
            <div className="bg-slate-50 p-4 border border-black mb-6">
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <span className={`${isA5 ? 'text-[8px]' : 'text-[9px]'} font-black text-gray-500 uppercase tracking-widest block mb-1`}>Customer Account:</span>
                        <p className={`${isA5 ? 'text-sm' : 'text-lg'} font-black uppercase text-black`}>{customer.name}</p>
                        {customer.address && <p className={`${isA5 ? 'text-[8px] mt-0.5' : 'text-[10px] mt-1'} opacity-70`}>{customer.address}, {customer.area}</p>}
                        {customer.phone && <p className={`${isA5 ? 'text-[8px] mt-0.5' : 'text-[10px] mt-1'} font-bold`}>Phone: {customer.phone}</p>}
                        {customer.gstNumber && <p className={`${isA5 ? 'text-[8px] mt-0.5' : 'text-[10px] mt-0.5'} font-bold`}>GSTIN: {customer.gstNumber}</p>}
                    </div>
                    <div className="text-right">
                        <span className={`${isA5 ? 'text-[8px]' : 'text-[9px]'} font-black text-gray-500 uppercase tracking-widest block mb-1`}>Current Outstanding:</span>
            ...
            <table className={`w-full border-collapse border border-black mb-8 ${isA5 ? 'text-[9px]' : 'text-xs'}`}>
                <thead>
                    <tr className="bg-gray-100 font-black uppercase border-b border-black">
                        ...
                        <th className={`${isA5 ? 'p-1 w-24' : 'p-2 w-32'} border-r border-black text-right`}>Debit (+)</th>
                        <th className={`${isA5 ? 'p-1 w-24' : 'p-2 w-32'} border-r border-black text-right`}>Credit (-)</th>
                ...
                <tbody>
                    {(customer.ledger || []).map((entry, idx) => (
                        <tr key={entry.id} className="border-b border-gray-300 last:border-b-0">
                            ...
                            <td className={`${isA5 ? 'p-1' : 'p-2'} border-r border-black text-right font-bold text-red-700`}>{entry.debit > 0 ? entry.debit.toFixed(2) : '-'}</td>
                            <td className={`${isA5 ? 'p-1' : 'p-2'} border-r border-black text-right font-bold text-emerald-700`}>{entry.credit > 0 ? entry.credit.toFixed(2) : '-'}</td>
                ...
                <tfoot>
                    <tr className="border-t-2 border-black bg-gray-100 font-black">
                        ...
                        <td className={`${isA5 ? 'p-1' : 'p-2'} border-l border-black text-right text-red-700`}>₹{(customer.ledger?.reduce((s, e) => s + (e.debit || 0), 0) || 0).toFixed(2)}</td>
                        <td className={`${isA5 ? 'p-1' : 'p-2'} border-l border-black text-right text-emerald-700`}>₹{(customer.ledger?.reduce((s, e) => s + (e.credit || 0), 0) || 0).toFixed(2)}</td>
