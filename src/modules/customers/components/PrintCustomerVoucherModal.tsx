...
const PrintCustomerVoucherModal: React.FC<PrintCustomerVoucherModalProps> = ({
    ...
    const resolvedInvoiceNo = (() => {
        ...
        if (refId && transactions && transactions.length > 0) {
            const found = transactions.find(t => t.id === refId);
            if (found) return found.invoiceNumber;
        }
        return formatVoucherNo(refNo) || '-';
    })();

    const receiptAgainstInvoice = resolvedInvoiceNo;
                        ...
                        <table className={`w-full border-collapse border border-gray-300 mb-6 ${isA5 ? 'text-[9px]' : 'text-xs'}`}>
                            <tbody>
                                <tr className="border-b border-gray-300">
                                    <td className={`w-1/3 font-bold bg-gray-50 border-r border-gray-300 text-gray-700 ${isA5 ? 'p-1.5' : 'p-2.5'}`}>Customer Name</td>
                                    <td className={`text-gray-900 font-medium ${isA5 ? 'p-1.5' : 'p-2.5'}`}>{customer.name}</td>
                                </tr>
                                <tr className="border-b border-gray-300">
                                    <td className={`font-bold bg-gray-50 border-r border-gray-300 text-gray-700 ${isA5 ? 'p-1.5' : 'p-2.5'}`}>Customer Address</td>
                                ...
                                {customer.gstNumber && (
                                    <tr className="border-b border-gray-300">
                                        <td className={`font-bold bg-gray-50 border-r border-gray-300 text-gray-700 ${isA5 ? 'p-1.5' : 'p-2.5'}`}>Customer GSTIN</td>
                                        <td className={`text-gray-900 font-medium ${isA5 ? 'p-1.5' : 'p-2.5'}`}>{customer.gstNumber}</td>
                                    </tr>
                                )}
                                <tr className="border-b border-gray-300">
                                    <td className={`font-bold bg-gray-50 border-r border-gray-300 text-gray-700 ${isA5 ? 'p-1.5' : 'p-2.5'}`}>Receipt Against Invoice No.</td>
                                    <td className={`text-gray-900 font-bold ${isA5 ? 'p-1.5' : 'p-2.5'}`}>{receiptAgainstInvoice}</td>
                                </tr>
                                <tr className="border-b border-gray-300">
                                ...
                                <tr className={`border-b border-gray-300 bg-gray-50 font-bold ${isA5 ? 'text-xs' : 'text-sm'}`}>
                                    <td className={`border-r border-gray-300 text-gray-900 ${isA5 ? 'p-1.5' : 'p-2.5'}`}>Amount Received</td>
                                    <td className={`text-gray-900 ${isA5 ? 'p-1.5' : 'p-2.5'}`}>₹{amountReceived.toFixed(2)}</td>
                                </tr>
                                <tr className="border-b border-gray-300">
                                ...
                                <tr className="border-b border-gray-300">
                                    <td className={`font-bold bg-gray-50 border-r border-gray-300 text-gray-700 ${isA5 ? 'p-1.5' : 'p-2.5'}`}>Auto Receipt</td>
                                    <td className={`text-gray-600 ${isA5 ? 'p-1.5' : 'p-2.5'}`}>{isAutoReceipt ? 'Yes (System Generated)' : 'No (Manual)'}</td>
                                </tr>
                                <tr>
