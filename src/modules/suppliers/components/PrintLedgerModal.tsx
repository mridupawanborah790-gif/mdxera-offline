    ...
    <div id="print-distributor-ledger-modal-container" className="fixed inset-0 bg-black/60 z-[1000] flex justify-center items-center backdrop-blur-sm print:bg-white print:p-0 overflow-y-auto">
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-5xl flex flex-col max-h-[95vh] print:max-h-none print:shadow-none print:rounded-none">
        ...
        <div className="flex-1 overflow-y-auto bg-gray-50 p-8 print:p-0 print:bg-white">
          <div id="ledger-print-area" className={`bg-white shadow-sm mx-auto print:shadow-none min-h-[297mm] w-full text-black font-sans ${isA5 ? 'p-3 max-w-[148mm] min-h-[210mm]' : 'p-5 max-w-[210mm] min-h-[297mm]'}`}>
            <div className="flex flex-col items-center border-b-2 border-black pb-3 mb-4 text-center">
                ...
                <div className="w-full flex justify-between items-center mt-3 pt-1.5 border-t border-dashed border-black">
                    <h2 className={`${isA5 ? 'text-[9px] px-1.5 py-0.5' : 'text-xs px-2.5 py-0.5'} font-black uppercase tracking-widest bg-black text-white`}>Ledger Statement</h2>
                    <p className={`${isA5 ? 'text-[8px]' : 'text-[10px]'} font-bold`}>Print Date: {new Date().toLocaleDateString('en-GB')}</p>
                </div>
            </div>

            <div className="bg-gray-100 p-3 border border-black mb-4">
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <span className={`${isA5 ? 'text-[8px]' : 'text-[9px]'} font-black text-gray-500 uppercase tracking-widest block mb-0.5`}>Account Ledger of:</span>
                        <p className={`${isA5 ? 'text-xs' : 'text-base'} font-black uppercase text-black`}>{distributor.name}</p>
                        {distributor.address && <p className={`${isA5 ? 'text-[8px] mt-0.5' : 'text-[10px] mt-0.5'} opacity-70`}>{distributor.address}</p>}
                        {distributor.gst_number && <p className={`${isA5 ? 'text-[8px] mt-0.5' : 'text-[10px] mt-0.5'} font-bold`}>GSTIN: {distributor.gst_number}</p>}
                    </div>
                    <div className="text-right">
                        <span className={`${isA5 ? 'text-[8px]' : 'text-[9px]'} font-black text-gray-500 uppercase tracking-widest block mb-0.5`}>Current Balance:</span>
            ...
            <table className={`w-full border-collapse border border-black mb-6 ${isA5 ? 'text-[8px]' : 'text-[11px]'}`}>
                <thead>
                    <tr className="bg-gray-200 font-black uppercase border-b border-black">
                        ...
                        <th className={`${isA5 ? 'py-1 px-1 w-16' : 'py-1.5 px-2 w-24'} border-r border-black text-right`}>Billed (+)</th>
                        <th className={`${isA5 ? 'py-1 px-1 w-16' : 'py-1.5 px-2 w-24'} border-r border-black text-right`}>Paid (-)</th>
                ...
                <tbody>
                    {(distributor.ledger || []).map((entry, idx) => (
                        <tr key={entry.id} className="border-b border-gray-300 last:border-b-0">
                            ...
                            <td className={`${isA5 ? 'py-1 px-1' : 'py-1.5 px-2'} border-r border-black text-right font-bold text-red-700 whitespace-nowrap`}>{entry.credit > 0 ? entry.credit.toFixed(2) : '-'}</td>
                            <td className={`${isA5 ? 'py-1 px-1' : 'py-1.5 px-2'} border-r border-black text-right font-bold text-emerald-700 whitespace-nowrap`}>{entry.debit > 0 ? entry.debit.toFixed(2) : '-'}</td>
                ...
                <tfoot>
                    <tr className="border-t-2 border-black bg-gray-100 font-black">
                        ...
                        <td className={`${isA5 ? 'py-1 px-1' : 'py-1.5 px-2'} border-l border-black text-right text-red-700`}>₹{(distributor.ledger?.reduce((s, e) => s + (e.credit || 0), 0) || 0).toFixed(2)}</td>
                        <td className={`${isA5 ? 'py-1 px-1' : 'py-1.5 px-2'} border-l border-black text-right text-emerald-700`}>₹{(distributor.ledger?.reduce((s, e) => s + (e.debit || 0), 0) || 0).toFixed(2)}</td>
